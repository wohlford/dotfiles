...
test_happy_path_links_core_files() {
  ...
  assert '.inputrc is a symlink into the fixture' \
    test "$(readlink "$home/.inputrc")" = "$fx/.inputrc"
}
...
test_relink_reports_old_target() {
  ...
  printf 'old inputrc\n' > "$home/elsewhere-dir/inputrc"
  ln -s "$home/elsewhere-dir/inputrc" "$home/.inputrc"
  local out
